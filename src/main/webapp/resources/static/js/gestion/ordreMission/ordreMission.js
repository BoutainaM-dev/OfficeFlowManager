app.controller('ordreMissionController', function($scope, $http, $location, $compile) {
	...
	$scope.init = function(){
		$scope.table = $('#ordreMissionTable').DataTable( {
			"oLanguage" : language,
			"ajax": "/gestion/ordreMission/rest/list",
			"columns": [
...
//				{ "data": "statut"},
	...
	$scope.add = function(){
		$scope.dto = {};
		$('#ordreMissionModal').modal('show');
	}
	
	$scope.preview = function(id){
		$http.get("/gestion/ordreMission/rest/load/"+id).then(function (response) {
	...
	$scope.confirmCancel = function(id) {
		$http.get("/gestion/ordreMission/rest/cancel/"+id).then(function (response) {
			$scope.table.ajax.reload();
		}, function error(response) {
	...
	$scope.validate = function(){
		$http.post("/gestion/ordreMission/rest/validate",$scope.dto).then(function (response) {
